...
const boxToTailwindConfig = (
  boxShadowsObj: typeof boxShadowsTailwindInline
) => {
  return Object.fromEntries(
    Object.entries(boxShadowsObj).map(([key, value]) => [
      key,
      `${value
        .replace(/shadow-\[/, '')
        .replace(/_/g, ' ')
...
const dropToTailwindConfig = (
  dropShadowsObj: typeof dropShadowsTailwindInline
) => {
  return Object.fromEntries(
    Object.entries(dropShadowsObj).map(([key, value]) => [
      key,
      `${value
        .replace(/drop-shadow-\[/, '')
        .replace(/_/g, ' ')
