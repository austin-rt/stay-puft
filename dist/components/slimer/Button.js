...
const Button = ({ id = undefined, callback, ariaControls, ariaExpanded, disabled = false, loading = false, ghost = false, title = '', theme = '', rounded = false, size = 'md', variant = 'default', leadingIcon, trailingIcon, loadingText = 'Loading...', loadingIcon = 'fa-solid fa-circle-notch fa-spin', ariaLabel, type = 'button', srDesc = '', }) => {
    return (React.createElement("button", { "data-c": "slmr--Button", id: id, type: type, disabled: loading || disabled, "aria-label": ariaLabel, "aria-controls": ariaControls, "aria-expanded": ariaExpanded, onClick: callback, className: `${rounded ? 'rounded-full' : 'rounded-md'} ${button.size[size]} ${disabled === true && loading !== true ? disabled : ''} ${ghost
